...
Reskin flavor names get mapped to the canonical card name. Secret Lair drops do things like ship `Unstable Harmonics`, which Moxfield and Archidekt treat as a distinct card but Cockatrice only knows as `Rhystic Study`. cod-sync resolves these against Scryfall's `/cards/collection` endpoint in a single batched POST per sync and caches the result at `~/.cache/cod-sync/alt_names.json`, so the second sync of a deck never re-queries. A small bundled seed covers the most common reskins so a fresh install with no network still maps them. Set `COD_SYNC_NO_NETWORK=1` to skip Scryfall entirely; unknown flavor names then pass through unchanged, which Cockatrice will reject on import.
...
The codebase is small enough to read in one sitting. `cli.py` holds the positional dispatcher, the directory walk, the unified per-file sync, and the bare-URL flow. `cod.py` is the format-preserving parser and writer. `diff.py` computes the per-zone change list and handles DFC name matching. `sourcetag.py` manages the URL marker in `<comments>`. `alt_name.py` resolves Secret Lair flavor names against Scryfall with a disk-backed cache. Each source lives in its own file under `sources/`, with the shared `RemoteDeck` type in `sources/types.py`. Tests sit next to the code and cover round-trip fidelity, diffing, multi-printing edits, URL stash behavior, DFC normalization, flavor-name resolution, the dispatcher, single-file sync, and bare-URL creation.
