...
Reskin flavor names get mapped to the canonical card name. Secret Lair drops do things like ship `Unstable Harmonics`, which Moxfield and Archidekt treat as a distinct card but Cockatrice only knows as `Rhystic Study`. cod-sync ships a bundled dictionary of every Scryfall reskin known at release time (currently 446 pairs, ~22KB) generated by `scripts/refresh_seed.py`. Lookups are pure in-memory dict reads — no network calls, no disk cache, no first-sync tax. Reskins released after a cod-sync release aren't covered until the seed is regenerated; if you hit one, re-run the refresh script and submit a PR or open an issue.
