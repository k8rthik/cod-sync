...
|---------|--------|------------------------------------------------------------------------------|
| 0.18.0  | MINOR  | New `--include-maybeboard` (`-m`) flag folds each source's maybeboard into the sideboard instead of dropping it (Moxfield maybeboard, Archidekt `includedInDeck: false`, ManaBox board 5); default behavior is unchanged. Ships a generated `man/cod-sync.1` (from `scripts/gen_manpage.py`, guarded against drift by `tests/test_manpage.py`) and enriches `--help` with a usage-forms + examples epilog. New flag = additive CLI grammar, hence MINOR. |
| 0.17.0  | MINOR  | The directory walk's free-text source prompt no longer accepts a lowercase `s` as a skip shortcut — it advertised only `empty=skip, q=quit`, so `s` now flows through as a typed source. Also hardens the walk summary: an unaccounted sync status now raises instead of being silently dropped from the footer (no user-visible change today, since walk can't produce one). Observable CLI-grammar change on the `s` input, hence MINOR. |
| 0.16.0  | MINOR  | ManaBox (`manabox.app/decks/<id>`) is now a supported deck source. ManaBox has no public API, so the deck is scraped from its Astro-rendered share page; commander/sideboard cards route to `side`, maybeboard is excluded, and multi-face names are layout-shaped like the other sources. Also: Ctrl-C now exits cleanly (a short stderr notice, status 130) instead of dumping a traceback. New source module plus a behavioral change on the interrupt path — additive CLI contract, hence MINOR. |
| 0.15.0  | MINOR  | Alt-name mappings are now logged during sync and prompt once per new mapping (accept / keep printed name / type a replacement), with answers persisted to the disk cache — in-flow control replacing the planned standalone override command. New `i` review answer ignores a card permanently via a `cod-sync-ignore:` comment marker; ignored cards are filtered from the diff and shown as dimmed notes. New prompt keys, new marker line, new log output — additive CLI/.cod contract changes, hence MINOR. |
| 0.14.2  | PATCH  | `python -m cod_sync` now propagates `main()`'s exit code instead of always exiting 0; the `cod-sync` console script was already correct. PATCH per the "obviously broken" rule — exiting 0 on a reported error is plainly a bug (user-confirmed tier). |
| 0.14.1  | PATCH  | Documentation only: new ARCHITECTURE.md design doc (data flow, name shaping incl. the reversible-printing edge case, cache schema, threading); module docstrings slimmed to purpose + contract; README/CONTRIBUTING drift fixed (commander-zone routing, stale `cli.py` layout, seed count, missing `--quiet`). No behavior change. |
| 0.14.0  | MINOR  | Legacy alt-name disk caches (written before layout-aware shaping) no longer resurrect full `A // B` names for true DFCs: marker-less cache files get every full-form value dropped on load and are rewritten with a `"__schema__": "2"` marker entry; marked files are trusted verbatim. Fixes poisoned pre-0.8.0-era entries writing unloadable DFC names into the .cod. The cache schema gains the reserved marker key — additive within the `str -> str` shape, hence MINOR not MAJOR. |
| 0.13.0  | MINOR  | Adventures (including Tarkir omens) and prepare cards now keep their full `A // B` names, matching Cockatrice's database — verified against cards.xml, which has no front-face-only entries for those layouts, so the old front-face output was unloadable. Changes what gets written to disk for affected cards; stale local shapes heal as remove + add on next sync. |
| 0.12.0  | MINOR  | Multi-face names are now shaped by the card's layout: split-style cards (split, aftermath, Duskmourn Rooms) keep their full `A // B` name instead of being wrongly reduced to the front face — changes what gets written to disk for those cards. Archidekt zone routing follows a card's primary category only, so secondary labels can't re-zone or exclude a card. Change summaries count cards (4x Forest = 4) instead of unique names, matching the create prompt. Plus: fetch announcements, single fetch on bare-URL import, shared lazy HTTP session. |
